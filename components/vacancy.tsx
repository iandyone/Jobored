...
const Vacancy: FC<IVacancyProps> = ({ loading, vacancy }) => {
  ...
  if (loading) {
    return (
      <div className={`${styles.vacancy} ${styles.loading}`}></div>
    );
  }
      ...
      {vacancy && (
        <>
          <div className={`${styles.vacancy__title} ${styles.title}`}>
            <h2 className={styles.title__text}><span>{vacancy.profession || "title"}</span></h2>
